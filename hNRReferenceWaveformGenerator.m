...
%   model specification used is taken from TS 38.141-1 v16.7.0.
% 
%   WG = hNRReferenceWaveformGenerator(RC,BW,SCS,DM,NCELLID,SV,CS) creates
...
%   values. The additional optional inputs, SV and CS, are described below.
%  
...
%   of TS 38.141-1 and TS 38.141-2 used ('15.1.0', '15.2.0', '15.7.0', '16.7.0').
...
%   FR2BandwidthTable - FR2 transmission bandwidth configurations (TS 38.104 Table 5.3.2-2)
...
classdef hNRReferenceWaveformGenerator
    ...
    properties (Constant)
        ...
        FR2BandwidthTable = getFR2BandwidthTable();               % FR2 transmission bandwidth configurations (TS 38.104 Table 5.3.2-2)
    ...
    methods        
        ...
        % Class constructor
        function obj = hNRReferenceWaveformGenerator(rc,bw,scs,duplexmode,ncellid,sv,cs)
        ...
        %   from the associated bandwidth configuration table (FR1 or FR2).    
            ...
            if contains(rc,["TM3.1" "TM3.1a"],'IgnoreCase',true)           % "NR-FR1-TM3.1", "NR-FR1-TM3.1a", "NR-FR2-TM3.1", "NR-FR2-TM3.1a"
                targets = [0 2];    
            elseif contains(rc,["TM3.2", "TM3.3"],'IgnoreCase',true)       % "NR-FR1-TM3.2", "NR-FR1-TM3.3"
                targets = 1;
            elseif contains(rc,["TM1.2", "TM2", "TM2a"],'IgnoreCase',true) % "NR-FR1-TM1.2", "NR-FR1-TM2", "NR-FR1-TM2a", "NR-FR2-TM2", "NR-FR2-TM2a"
                targets = 2;
            else
                targets = 0; 
            end 
...
function table = getFR1BandwidthTable()
    ...
    nrbtable = [25  52 79 106 133 160 216 270 NaN NaN NaN NaN NaN;     % 15 kHz
     ...
     % Package NRB array into a table (column names must be valid MATLAB identifiers, thus the BW_ prefix)    
...
function table = getFR2BandwidthTable()    
    % TS 38.104 Table 5.3.2-2:   Transmission bandwidth configuration NRB for FR2
    % TS 38.101-2 Table 5.3.2-1: Maximum transmission bandwidth configuration NRB (FR2)
    % NRB, for BW and SCS
    % BW MHz    50 100 200 400
    nrbtable = [66  132 264 NaN;   % 60 kHz
                32  66  132 264];  % 120 kHz
...
function p = getRMConfigs()
    ...
    % 'NR-FR1-TM1.1','NR-FR1-TM1.2','NR-FR1-TM2','NR-FR1-TM2a','NR-FR1-TM3.1','NR-FR1-TM3.1a','NR-FR1-TM3.2','NR-FR1-TM3.3' 
...
function [config,txpattern] = getTDDConfiguration(fr,scs,link,dlfrc)
    ...
    if nargin < 4
        dlfrc = 0;
        % Default to NR-TM configs, if FRC flag is not supplied
        if nargin < 3
            link = "downlink"; 
        end   
    end
    ...
    if isempty(patterns)
        ...
        % 3D1S1U, S=10D:2G:2U (5 slots = 5ms)
        ...
        % 7D1S2U, S=6D:4G:4U (10 slots = 5ms)
        ...
        % 14D2S4U, S=12D:4G:0U & S=0D:6G:8U (20 slots = 5ms)
        ...
        % NR-TM 
        % FR2 (60kHz,120kHz)
        % 3D1S1U, S=10D:2G:2U (5 slots = 1.25ms)
        % 60 kHz
        ...
        % 120 kHz
        % 7D1S2U, S=6D:4G:4U (10 slots = 1.25ms)
        ...
        % 60 kHz
        % 3D1S1U, S=4D:6G:4U (5 slots = 1.25ms)
        ...
        % 120 kHz
        % 3D1S1U, S=10D:2G:2U (5 slots = 0.625ms)
        ...
        % 60 kHz/120 kHz SCS: 3D1S1U, S=10D:2G:2U (5 slots = 1.25ms or 0.625ms)
    ...
    if link == "downlink"
        ...
        ulselection = [1 2 7 7];  % Patterns for uplink FRC @ 15,30,60,120 kHz SCS
        index = ulselection(scsidx);
        config = patterns(index);
        config.referenceSubcarrierSpacing = scs;
        config.dl_UL_TransmissionPeriodicity = (1+double(scsidx==3))*config.dl_UL_TransmissionPeriodicity;  % Re-adjust the period for 60kHz case 
    end
    
    % DL-UL period pattern, marked out in a vector (0=DL, 1=S containing DL, 2=UL)
    slotspertddperiod = config.dl_UL_TransmissionPeriodicity*fix(scs/15);          % Number of slots in the TDD DL-UL period ( slots in 1ms)
    txpattern = 2*ones(1,slotspertddperiod);                                     % Mark all slots with 2
...
function waveconfig = getCommonParameters(tdef,bw,scs,ncellid)
    ...
    % scs      =  15, 30, 60, 120,240           % If 60kHz and FR2 then "D" (adjusted below)
    % ssbscs   =  15, 30, 30, 120,240
    ssbcases   = ["A","B","B","D","E"];
    ...
    ssbbmap(1) = 1;                          % Enable the first occurrence 
    ...
    if strcmpi(tdef.DuplexMode,'FDD')
        ...
        % For TDD, use downlink and downlink partial slots     
        [~,dltxpattern] = getTDDConfiguration(tdef.FR,scs,"downlink",frc);
        dlallocatedslots = find(dltxpattern<(2-frc))-1;      % Schedule in DL slots for FRC, and both DL and special slots for TM
...
function waveconfig = getFRCPDSCHParameters(tdef,waveconfig)
    ...
    if  waveconfig.SSBurst.BlockPattern <= "Case C" && ...    % SSB SCS <= 30kHz
...
function waveconfig = getTestModelPDSCHParameters(tdef,waveconfig,sv)
    ...
    % Using Rel 15/16 badged versions of 38.141: 15.0.0, 15.1.0, 15.2.0... 15.7.0... 16.7.0
    % Identify the std version of NR-TM required
    supportedversions = ["15.1.0", "15.2.0", "15.7.0", "16.7.0"];
...
% TS 38.214 Table 5.1.3.1-1 (qam64) and Table 5.1.3.1-2 (qam256)
...
function plotResourceGrid(~, waveconfig, cfgObj, gridset, isDownlink)
    ...
    for bp = 1:length(gridset)
        ...
        if isDownlink
            ...
            if ~isempty(thisRsv)
                nsymbolsperhalfframe = thisRsv.Period*14;  % Always NCP
